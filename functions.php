...
function dgm_get_service_hero_image(string $page_key): ?array {
    ...
    $slot      = (int) floor(($now - $anchor) / DAY_IN_SECONDS);
    $cache_key = 'dgm_hero_slot_' . $slot;
    ...
    if ($cached === false) {
        ...
        foreach ($ids as $id) {
            ...
            if (!empty($meta['width']) && $meta['width'] > 1000) {
                $large[] = $id;
            }
        }

        if (empty($large)) return null;

        mt_srand($slot * 31337 + count($large));
        shuffle($large);
        mt_srand();

        $page_keys = ['seo', 'google-ads', 'contentmarketing', 'website'];
        $result    = [];
        foreach ($page_keys as $i => $key) {
            $id           = $large[$i % count($large)];
            $meta         = wp_get_attachment_metadata($id);
            $result[$key] = [
                'url'    => wp_get_attachment_image_url($id, 'full') ?: wp_get_attachment_url($id),
                'width'  => $meta['width']  ?? 1200,
                'height' => $meta['height'] ?? 660,
            ];
        }
